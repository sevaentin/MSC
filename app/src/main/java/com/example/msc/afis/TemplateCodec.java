...
import com.example.msc.afis.fingerprintio.TemplateFormat;
import com.example.msc.afis.fingerprintio.*;
import com.example.msc.afis.fingerprintio.ansi378.*;
import com.example.msc.afis.fingerprintio.iso19794p2.*;
...
abstract class TemplateCodec {
	...
	private static class Ansi378Codec extends TemplateCodec {
		@Override byte[] encode(List<MutableTemplate> templates) {
			...
			Integer width = 0;
			for(MutableTemplate val : templates) {
				if(val.size.x > width)
					width = val.size.x;
			}
			iotemplate.width = width > 0 ? width : 500;
			...
			iotemplate.height = height > 0 ? height : 500;
